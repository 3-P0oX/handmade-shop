...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['signup'])) {
    ...
    $check_email_query = "SELECT * FROM merchants WHERE email = '$email'";
    ...
    if ($check_email_result->num_rows > 0) {
        ...
        if ($password !== $confirm_password) {
            ...
            $insert_merchants_query = "INSERT INTO merchants (merchant_name, email, password, number, about_him) VALUES ('$name', '$email', '$hashed_password', '$number', '$about_you')";
            $insert_merchants_result = $conn->query($insert_merchants_query);

            if ($insert_merchants_result) {
                // تم إضافة المستخدم بنجاح
                $success = "تم إنشاء حساب المستخدم بنجاح";
                header("Location: product-merchant.php");
                exit();
            } else {
    ...
    <div class="form-container">
        ...
        <div class="form-content sign-in-form active">
            ...
            <form action="#" method="POST">
                ...
                <textarea name="about_you" id="about_you" cols="50" rows="10" placeholder=" تحدث عن نفسك" autofocus required></textarea>
